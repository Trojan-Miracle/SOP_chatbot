...
def prepare_messages(messages: Sequence[Message | BaseMessage], system_prompt: str) -> list[Message | BaseMessage]:
    ...
    # model_construct bypasses Message's max_length=3000 validator — that cap
    # exists to bound user-submitted ChatRequest content, not the system
    # prompt, which legitimately grows past it once RAG context is embedded.
