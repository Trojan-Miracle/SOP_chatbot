...
async def lifespan(app: FastAPI):
    ...
    # Pre-warm the embedding model + Chroma collection: loading the
    # sentence-transformers weights takes seconds, so do it once here instead
    # of on the first /documents/upload or /chat request
    try:
        await asyncio.to_thread(get_vectorstore)
        await asyncio.to_thread(get_bm25_index().rebuild)
        logger.info("embedding_model_and_vectorstore_pre_warmed")
    except Exception as e:
...
# Minimal static test UI (login/upload/chat) — served at /ui, same origin as
# the API so it never needs CORS configuration
