...
class LLMService:
        ...
        **model_kwargs: Any,
        ...
        Args:
            ...
            response_format: Pydantic schema for structured output. When
                ...
                raw ``BaseMessage``.
            config: Optional ``RunnableConfig`` (callbacks, metadata) forwarded
                to the underlying ``.ainvoke()`` call — pass the node's own
                ``config`` through here so tracing callbacks (e.g. Langfuse)
                actually see these calls instead of only the graph-level span.
        ...
        try:
            return await asyncio.wait_for(
                self._call_with_fallback(messages, model_name, response_format, config, model_kwargs),
    ...
    async def _invoke_with_retry(
        ...
        Args:
            ...
            config: Optional ``RunnableConfig`` forwarded to ``.ainvoke()``.
    ...
    async def _call_with_fallback(
        ...
        def _override_target(idx: int) -> Any:
            ...
            # method="function_calling" — DeepSeek (and most OpenAI-compatible
            # gateways) reject the strict json_schema response_format that
            # with_structured_output() defaults to for newer OpenAI models.
            return base.with_structured_output(response_format, method="function_calling") if response_format else base
    ...
    async def _fallback_loop(
        ...
        Args:
            ...
            config: Optional ``RunnableConfig`` forwarded to each attempt.
