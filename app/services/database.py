...
class DatabaseService:
    ...
    async def create_document(self, document_id: str, filename: str, file_path: str) -> Document:
        ...
        with Session(self.engine) as session:
            ...
            logger.info("document_created", document_id=document_id, filename=filename)
            return document

    async def update_document_status(
        self, document_id: str, status: DocumentStatus, chunk_count: int = 0, error: str | None = None
    ) -> Optional[Document]:
        ...
        Args:
            ...
            chunk_count: Number of vector chunks produced (set when status is READY)
        ...
        with Session(self.engine) as session:
            ...
            session.commit()
            session.refresh(document)
            logger.info("document_status_updated", document_id=document_id, status=status.value)
            return document
    ...
    async def get_documents(self) -> List[Document]:
        ...
        Returns:
            List[Document]: All document records
        """
        with Session(self.engine) as session:
            statement = select(Document).order_by(col(Document.created_at).desc())
