"""Shared SOP retrieval for graph, tool-calling and investigation workflows."""
...
async def search_sop(query: str, document_ids: list[str] | None = None) -> list[RetrievedChunk]:
    """Search SOP chunks using the shared vector/BM25/RRF configuration.

    The optional document filter narrows retrieval, not authorization. All
    callers receive the same ranking and source metadata for identical inputs.
    """
    vectorstore = get_vectorstore()
    doc_filter = {"document_id": {"$in": document_ids}} if document_ids else None
