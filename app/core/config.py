...
class Settings:
    ...
    def __init__(self):
        ...
        self.PROJECT_NAME = os.getenv("PROJECT_NAME", "SOP Copilot")
        self.VERSION = os.getenv("VERSION", "1.0.0")
        self.DESCRIPTION = os.getenv("DESCRIPTION", "Evidence-grounded SOP assistance with versioned human approval")
        ...
        # LangGraph / LLM Configuration (DeepSeek, OpenAI-compatible)
        ...
        self.GRADE_LLM_MODEL = os.getenv("GRADE_LLM_MODEL", "deepseek-v4-flash")
        ...
        self.CHROMA_COLLECTION_NAME = os.getenv("CHROMA_COLLECTION_NAME", "sop_documents")
        self.RAG_CHUNK_SIZE = int(os.getenv("RAG_CHUNK_SIZE", "800"))
        self.RAG_CHUNK_OVERLAP = int(os.getenv("RAG_CHUNK_OVERLAP", "100"))
        self.RAG_TOP_K = int(os.getenv("RAG_TOP_K", "5"))
        self.RAG_MAX_REWRITES = int(os.getenv("RAG_MAX_REWRITES", "2"))
        self.UPLOAD_DIR = os.getenv("UPLOAD_DIR", "./data/uploads")
        self.MAX_UPLOAD_SIZE_MB = int(os.getenv("MAX_UPLOAD_SIZE_MB", "20"))

        # Hybrid retrieval (vector + BM25 keyword, fused via reciprocal rank
        # fusion) and a distance-based pre-filter on the vector side. Defaults
        # empirically derived from this project's own test runs: relevant
        # hits scored ~0.47-0.6, off-topic ones ~0.9-1.4.
        self.RAG_HYBRID_SEARCH_ENABLED = os.getenv("RAG_HYBRID_SEARCH_ENABLED", "true").lower() == "true"
        self.RAG_CANDIDATE_K = int(os.getenv("RAG_CANDIDATE_K", "15"))
        self.RAG_RRF_K = int(os.getenv("RAG_RRF_K", "60"))
        self.RAG_SCORE_THRESHOLD = float(os.getenv("RAG_SCORE_THRESHOLD", "1.2"))

        # Long-term memory Configuration (mem0 — DeepSeek LLM + local HF embedder,
        # since DeepSeek has no embeddings endpoint and mem0 needs its own)
        self.LONG_TERM_MEMORY_ENABLED = os.getenv("LONG_TERM_MEMORY_ENABLED", "true").lower() == "true"
        self.LONG_TERM_MEMORY_MODEL = os.getenv("LONG_TERM_MEMORY_MODEL", self.GRADE_LLM_MODEL)
        self.LONG_TERM_MEMORY_EMBEDDER_MODEL = os.getenv("LONG_TERM_MEMORY_EMBEDDER_MODEL", self.EMBEDDING_MODEL)
        self.LONG_TERM_MEMORY_EMBEDDER_DIMS = int(os.getenv("LONG_TERM_MEMORY_EMBEDDER_DIMS", "1024"))
        ...
        default_endpoints = {
            ...
            "documents_upload": ["10 per minute"],
            "documents_list": ["30 per minute"],
        }
