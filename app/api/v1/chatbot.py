...
async def chat(
    ...
    try:
        ...
        result, sources = await agent.get_response(
        ...
        logger.info("chat_request_processed", session_id=session.id, source_count=len(sources))

        return ChatResponse(messages=result, sources=sources)
...
async def chat_stream(
    ...
    try:
        ...
        async def event_generator():
            ...
            """
            try:
                with llm_stream_duration_seconds.labels(model=llm_service.get_llm().get_name()).time():
                ...
                # Send final message indicating completion, with sources attached
                sources = await agent.get_last_sources(session.id)
                final_response = StreamResponse(content="", done=True, sources=sources)
