...
`--judge` 对两种策略的可回答方案额外执行相同的逐步骤支持评审，保存 `support_passed` 和逐条判定；评测调用单独记录为 `evaluation_model_calls`，不混入策略本身的调用计数与耗时。评审错误单独记录，不能当作模型通过。该评审与 adaptive 内部评审复用同类模型和标准，存在相关性偏差，不能替代人工金标。
